...
keys, values = zip(*args_grid.items())
...
# Add column names
table.field_names = ["Model", "Params", "Time", "Accuracy", "F1 Score"]

for combo in tqdm.tqdm(combinations):
    # Create a dummy argparse.Namespace object
    args = argparse.Namespace()
    # Populate args with the current combination
    ...
    result = test(test_loader,nclass)

    table.add_row([args.model, args.num_gene, 0, f"{result[0]:.2f}",  result[1]])
    print(result[0])
